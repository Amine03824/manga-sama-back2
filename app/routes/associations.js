...
router.route('/article/manga/:isbn')
  .get(articleController.getArticlesByManga);

// Associe un manga à un article par la table de relation manga_has_article
router.route('/article/manga/:articleId/:isbn')
  .post(articleController.linkOneMangaToOneArticle);


// Récupéprer toutes les annonces en lien avec un user particulier
router.route('/user/:userId/article/')
  .get(articleController.getArticlesByUser);

// Associe un utilisateur à un article par la table de relation user_has_article
router.route('/user/article/:userId/:articleId')
  .post(articleController.linkOneUserToOneArticle);
